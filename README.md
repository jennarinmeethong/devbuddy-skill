...
```text
codex plugin marketplace add jennarinmeethong/devbuddy-skill --ref v1.0.5 --json
...
```text
claude plugin marketplace add jennarinmeethong/devbuddy-skill@v1.0.5 --scope user --sparse .claude-plugin plugin/devbuddy-claude-code
...
These examples pin the release tag. Replace `v1.0.5` with an approved release
...
`v1.0.5` is the next package version in this repository; publish the matching
...
```text
git clone --depth 1 --branch v1.0.5 https://github.com/jennarinmeethong/devbuddy-skill.git devbuddy-release
