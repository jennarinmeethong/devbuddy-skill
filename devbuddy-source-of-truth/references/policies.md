...
- `tools.is_rtk` is an inherited dispatch constraint, not only an Orchestrator preference. Resolve it before every specialist dispatch and include `rtk_required: true|false` in the task package. When `rtk_required` is `true`, every specialist must use RTK for each supported delivery shell command; if `rtk` is unavailable, it must return `waiting_user` before running the direct equivalent. The Orchestrator must reject a task package that omits this field.
