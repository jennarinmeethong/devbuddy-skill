...
The task-memory tool is an enforcement boundary for file protocol, not an identity provider: adapters must also prevent specialists from receiving direct write access to `.devbuddy/`. Create a slice record only at a role boundary, a material checkpoint, or a blocker. The owner saves the returned JSON in `task-<task-id>/inbox/` before invoking `task_memory.py record`; the tool rejects inputs elsewhere. Validate it against `schemas/slice-record.schema.json`, persist it with `task_memory.py record`, and use JSON only—never Markdown. Records have no file-size or character cap; still dispatch only the `next_slice` data plus referenced files/keys. Put durable state in the ledger, references in paths/keys, and raw logs in files rather than in the record. All runtime-created files, including atomic-write staging files, remain below the selected `.devbuddy` root on macOS, Windows, and Linux.
