...
| `check_manual_conformance.py` | Verify the one central bilingual manual, its three host pages, links, anchors, version metadata, and stylesheet. | `python scripts/check_manual_conformance.py manual` |
| `check_semantic_conformance.py` | Detect drift in shared versions, roles, slice record fields, memory defaults, policy tokens, and platform transport mappings. | `python scripts/check_semantic_conformance.py` |
| `validate_skill_contract.py` | Validate explicit prompt behaviors for the source and both generated adapters without PyYAML. | `python scripts/validate_skill_contract.py` |
...
| `sync_adapter_skills.py` | Render the shared adapter prompt core and loading matrix, or detect drift. | `python scripts/sync_adapter_skills.py --check` |
