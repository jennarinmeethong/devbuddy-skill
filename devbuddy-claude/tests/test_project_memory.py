#!/usr/bin/env python3
"""Regression tests for the Claude memory layout and settings validator."""
...
class InstallerTests(unittest.TestCase):
    def test_installer_is_dry_run_by_default(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = run(INSTALLER, "--claude-root", claude_root)
            self.assertEqual(result.returncode, 0, result.stderr or result.stdout)
            self.assertFalse((claude_root / "skills" / "devbuddy").exists())
