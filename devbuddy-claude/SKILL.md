...
2. Resolve `.devbuddy/settings.yaml`, `workspace.projects`, and `knowledge-base/`. Validate settings before dispatch and use only the active Claude profile; never change settings to switch adapters. Read `tools.is_rtk`: when true use supported RTK equivalents; if an equivalent is required but `rtk` is unavailable, set the affected work `waiting_user`. Resolve and pass it to every specialist as `rtk_required`; reject any task package that omits it.
