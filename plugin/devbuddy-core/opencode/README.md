...
monorepo. Omit `--global` to install into the current project. For local
discovery, place or link this directory's `index.js` in `.opencode/plugins/`.
For npm distribution, publish `@devbuddy/opencode-plugin` and add the package
name to OpenCode's plugin configuration.
