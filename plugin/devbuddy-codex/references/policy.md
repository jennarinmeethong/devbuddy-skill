...
- Resolve `tools.is_rtk` before every delivery command and every specialist dispatch. Include the result as mandatory `rtk_required` in each task package. When it is `true`, the specialist uses RTK for every supported delivery shell command; if RTK is unavailable, it returns `waiting_user` rather than using the direct equivalent. Commands without an RTK equivalent may run directly.
