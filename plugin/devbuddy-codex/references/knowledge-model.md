...
New IDs use `<prefix>-<UTC timestamp>-<UUID4>`, generated by the installed Plugin: `python3 <installed-plugin-root>/scripts/new_knowledge_key.py --prefix INC`. The UUID component makes independently created `INC`, `BR`, and every other entity key collision-resistant across machines, branches, and repositories without a shared counter. Existing sequential IDs (for example `REQ-001`) are immutable and are never renamed merely to adopt the new format.
