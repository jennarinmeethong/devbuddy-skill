...
- A user may set an alternate memory root, including an Obsidian vault path. Treat the specified path as the DevBuddy memory root directly; do not add another `.devbuddy/` wrapper.
...
- Created the required Claude and Codex adapter checklist instances. Both adapters are represented as implemented only after their checklist and validation evidence pass.
...
- Codex dispatch blocks as `waiting_user` when the subagent capability cannot accept explicit `model`/`reasoning_effort`, settings/resource limits are missing, or a required approval/gate is incomplete. The Orchestrator never substitutes for a specialist.
...
- The Codex adapter bundles a platform-specific knowledge model and standard-library project-memory initialization/validation scripts; `--project-root` creates the canonical `<project-root>/.devbuddy/` wrapper, while `--root` supports approved external roots without nesting.

### Claude adapter implementation

- Implemented `devbuddy-claude/` as a self-contained Claude Code Skill invoked explicitly as `/devbuddy`, with the same command grammar as the Codex adapter, canonical roles and approved aliases, ranked project model/effort allowlists, standard-library validators, no-model-call scenario coverage, and a bilingual local manual.
- Claude Code carries reasoning effort in the agent-definition `effort` frontmatter field and allows the model to be overridden per Agent call. The adapter therefore installs one subagent per canonical role and effort tier — `devbuddy-<role>-{low,medium,high}`, 27 definitions — and passes the model explicitly on every dispatch. Selecting the tier is selecting the effort level, so `DBY-MODEL-002` is satisfied without a documented exception.
- Agent definitions must not pin a `model`; a pinned model would override the Orchestrator's recorded selection and make the ledger entry false. `scripts/validate_skill_metadata.py` rejects a pinned model, a missing `effort`, a name/filename mismatch, and any missing definition. `scripts/generate_agents.py` regenerates all 27 from `roles/` and verifies them with `--check`.
- Claude dispatch blocks as `waiting_user` when the required subagent is not installed, settings/resource limits are missing, no approved pair is sufficient, a required approval or gate is incomplete, or the effort level actually used cannot be verified. The Orchestrator never substitutes for a specialist.
- The adapter ships a ranked default allowlist (`haiku`/`sonnet`/`opus`, `low`/`medium`/`high`) so it is usable immediately. This is an approval boundary, not a cost waiver; `max_concurrency`, `task_timeout_seconds`, and `retry_limit` remain `user_required` because they commit the user's time and money.
- Policy is enforced through skill instructions only. The adapter does not modify `.claude/settings.json` and installs no hooks, so Claude Code's own permission prompts remain the user's backstop. Agent definitions inherit the default tool set rather than narrowing it per role, because every role legitimately writes some artefact and a tool-level lockdown would break artefact ownership instead of protecting it.
- `scripts/install_claude_adapter.py` installs the skill to `~/.claude/skills/devbuddy/` and the 27 subagents to `~/.claude/agents/`. Dry run is the default, and it refuses to overwrite any existing file it cannot identify as a DevBuddy artefact.
- Marked every Claude adapter checklist requirement `done` only with target location and validation evidence. Updated the source-of-truth Claude manual pages from "not implemented" to installation, configuration, dispatch, and troubleshooting guidance.
