...
- **SMOKE-007 passed 2026-08-03** — invocation gate and dispatch pair verified after adopting `disable-model-invocation: true`. A model-initiated `Skill` call on `devbuddy` was refused by the harness itself (`cannot be used with Skill tool due to disable-model-invocation`), confirming the `/devbuddy`-only rule is enforced by Claude Code rather than by skill prose; both installed copies of the skill carry the field. A real read-only `devbuddy-reviewer-low` dispatch with an explicit `model` then returned a structured handoff reporting `claude-haiku-4-5-20251001 / low`, so the split transport — effort from the agent definition, model from the Agent call — is confirmed end to end and `DBY-MODEL-002` verification is observed, not assumed. The reviewer stayed read-only (one tool use, no writes) and its findings were applied to `references/custom-tools.md`.
- The `claude` CLI is still absent from this machine, so headless triggering tests via `claude -p` remain unavailable. The gate was verified in-session instead, which tests the same enforcement path the user actually uses.
