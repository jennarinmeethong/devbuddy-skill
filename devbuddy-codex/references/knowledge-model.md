...
The automatic DevBuddy allowance covers help, validation, dry-run, repository inventory, and task-lifecycle calls for a current user-started task. It does not suppress a Codex platform permission prompt, authorize any external/destructive action, or remove the required gate for workspace writes and canonical knowledge.
