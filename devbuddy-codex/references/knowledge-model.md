...
Resolve `<project-root>/.devbuddy/settings.yaml` first. By default, that same `.devbuddy/` directory contains `settings.yaml`, the four core memory files, and the typed folders. A configured external `memory_root` is used directly; do not add another `.devbuddy/` wrapper there. Canonical memory is never global or temporary.
...
The memory root contains `Context.md`, `BusinessContext.md`, `DecisionLog.md`, `KnowledgeBase.md`, typed folders, and `tasks/` for task ledgers. Every canonical entity has an immutable typed key such as `REQ-001`, `BR-001`, `API-001`, `DB-001`, or `ADR-001`, plus owner, source/evidence, verification date, and confidence metadata.
...
Run `scripts/validate_knowledge.py <memory-root>` after material memory changes and before closure. It checks core files, entity metadata, key format, and duplicate IDs.
