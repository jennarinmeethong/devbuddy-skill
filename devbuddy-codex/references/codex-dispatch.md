...
Codex keeps role instructions independent from effort. Do not create a separate role file for each effort tier; select and verify the effort per dispatch.
...
The selected pair must be explicit in the dispatch and must be verifiable in the returned execution metadata or handoff. A generic subagent fallback, an omitted parameter, or an unverified parameter does not satisfy the contract.
